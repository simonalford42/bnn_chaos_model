    ...
    #y=y
        ...
        if config['sr_residual']:
            ...
            if flag == "middle":
                ...
                selected_complexities = {6, 12, 14, 21, 28, 29}
                selected_equations = []
...
#     print(f"Contents of X (first 5 rows):\n{X[:5]}")
...
#         # Get the complexity column from the previous run
#         complexities = previous_sr_model.equations_[0]['complexity'].values

#         # Map complexities to the respective input variables
#         complexity_of_variables = list(complexities[:X.shape[1]])

#         # Print the complexities and make sure they match the number of features
#         print(f"Complexity of variables: {complexity_of_variables}")
#         print(f"Number of complexities: {len(complexity_of_variables)}")
        
#         # Ensure the number of complexities matches the number of features
#         if len(complexity_of_variables) != num_features:
#             raise ValueError(f"Mismatch between complexities ({len(complexity_of_variables)}) and number of features ({num_features}).")
...
def run_pysr(config):
    ...
    if config.get('sr_residual', False):
        ...
        if flag == "top":
            ...
            # Filter mean equations with complexity 3, 6, 10, and 15
            selected_complexities = {6, 12, 14, 21, 28, 29}
            complexities_list = []
